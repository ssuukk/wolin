...
import pl.qus.kotlinek.exception.NoRuleException
import java.io.*
...
// gradlew generateGrammarSource
        ...
        parseWolinek(FileInputStream(File("src/main/kotlinek/test.ktk")))

        translateAsm(
            FileInputStream(File("src/main/kotlinek/assembler.asm")),
            FileInputStream(File("src/main/kotlinek/template.asm"))
        )
    }
    ...
    private fun translateAsm(asmStream: InputStream, templateStream: InputStream) {
        ...
        val ostream = BufferedOutputStream(FileOutputStream(File("src/main/kotlinek/assembler.s")))
...
//            translationVisitor.state.floats = declarationVisitor.state.floats
        ...
        val ostream = BufferedOutputStream(FileOutputStream(File("src/main/kotlinek/assembler.asm")))

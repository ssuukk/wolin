    ...
    override fun visitEqualityComparison(ctx: KotlinParser.EqualityComparisonContext): WolinStateObject {
        when {
            ...
            ctx.equalityOperation().size == 1 -> {
                ...
                state.freeReg("for comp right side, type =${state.currentKotlinekType}")
                state.freeReg("for comp left side, type =${state.currentKotlinekType}")
...
//    }
    ...
    fun processAssignment(ctx: ParseTree, leftFunction: () -> WolinStateObject, rightFunction: () -> WolinStateObject) {
        ...
        state.inferTopOregType()
        state.freeReg("for assigned variable, type = ${state.currentKotlinekType}")
    ...
    private fun processTwoSides(
        ...
        state.inferTopOregType()
        state.freeReg("for right side, type =${state.currentKotlinekType}")
    }
                    ...
                    ctx.postfixUnaryOperation().size == 1 -> {
                        ...
                        when {
                            ...
                            prawo.arrayAccess() != null -> {
                                ...
                                state.allocReg("arr_deref", state.currentKotlinekType)
        ...
        when {
            ctx.expression().size == 1 -> {
                ...
                if(state.currentShortArray == null) {
                    state.allocReg("For calculating index",state.currentKotlinekType)
                }
...
//    }
    ...
    override fun visitFunctionLiteral(ctx: KotlinParser.FunctionLiteralContext): WolinStateObject {
        ...
        nowaFunkcja.type = state.currentKotlinekType
...
//        print(test)
        ...
        """.trimMargin()
            ...
            state.code("let SPE(3)[ubyte] = SP[ubyte] // aktualny stos kotlinka")
            ...
            state.code("let SP[ubyte] = SPE(3)[ubyte] // przywrócenie stosu kotlinka")

...
class WolinStateObject(val pass: Pass) {
    ...
    //var currentKotlinekType: String = "unit"
    var currentKotlinekType = Typ.unit
    var labelCounter = 0
    var loopCounter = 0
    var lambdaCounter = 0
...
//    }
    ...
    fun setTopOregType(kotlinekType: Typ) {
        operStack.peek().type = kotlinekType
        code("// setTopOregType to $kotlinekType")
    }

    fun inferTopOregType() {
        operStack.peek().type = currentKotlinekType

        code("// inferTopOregType ${operStack.peek().name} -> $currentKotlinekType")
    }
